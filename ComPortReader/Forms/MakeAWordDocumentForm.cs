    ...
    public partial class MakeAWordDocumentForm : Form
    {
        ...
        private void makeAWordDocumentForm_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < readings.Count; i++)
            {
                ...
                if (currentReading.type == "Прямоугольная")
                {
                    
                    squareMetalSamplesTable.Rows.Add(currentReading.metalMarking, numbers.ElementAt(j)[0], currentReading.aDim, currentReading.bDim, currentReading.totalArea, currentReading.originalLength, currentReading.endLength, currentReading.maxForce, forceAtStressPointsStr, currentReading.tempTearResistance, stressPointStr, currentReading.relativeExpansion);

                }

                if (currentReading.type == "Круглая")
                {
                    roundMetalSamplesTable.Rows.Add(currentReading.metalMarking, numbers.ElementAt(j)[0], currentReading.diameterBefore,  currentReading.totalArea, currentReading.diameterAfter, currentReading.originalLength, currentReading.endLength, currentReading.maxForce, forceAtStressPointsStr, currentReading.tempTearResistance, stressPointStr, currentReading.relativeExpansion, currentReading.relativeNarrowing);
                }
        ...
        {
            string fileName = Parsing.CheckForDuplicateDOCX(defaultPath, defaultPath, protocolNumTB.Text + " ( " + MetalMarkings.Text + ")");
            ...
            l6.Append(" " + MetalMarkings.Text, defaultText);
            doc.InsertParagraph();
            ...
            {
                Xceed.Document.NET.Table roundMetalSamplesTableData = doc.AddTable(roundMetalSamplesTable.Rows.Count, roundMetalSamplesTable.Columns.Count - 1);


                for (int i = 0; i < roundMetalSamplesTable.RowCount; i++)
                {
                    for (int j = 0; j < roundMetalSamplesTable.Columns.Count - 1; j++)
                    {
                        if (i == 0)
                        {
            ...
            doc.InsertSectionPageBreak(); doc.InsertSectionPageBreak(); doc.InsertSectionPageBreak(); doc.InsertSectionPageBreak();
